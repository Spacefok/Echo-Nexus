...
LevelGenerator::LevelGenerator(ServiceLocator& locator) : locator_(locator) {}
...
void LevelGenerator::generateLevel(int levelId) {
    ...
    for (auto& d : data["drones"]) {
        ...
        int health = d.contains("health")
                         ? d.at("health").get<int>()
                         : enemyStats[d.at("type").get<std::string>()].value("health", 10);
        float speed = d.contains("speed")
                          ? d.at("speed").get<float>()
        ...
        auto drone =
            std::make_shared<Drone>(id, factions[factionName], d.at("type").get<std::string>(),
